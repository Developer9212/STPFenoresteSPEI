...
import fenoreste.spei.modeloEntrada.*;
import fenoreste.spei.modeloSalida.*;
...
public class OutServiceGeneral {
    ...
    public ResponseLocalDispersionVo sendOrder(RequestLocalDispersionVo order) {
        ...
        try {
            ...
            TablaPK tbpk = new TablaPK();

            boolean banderaProcesa = false;
            ClabeInterbancaria clabe = new ClabeInterbancaria();
            if (a != null) {
                if (a.getEstatus() == 2) {
                    if (origenService.buscarMatriz().getIdorigen() != 30500) {
                        //Buscamos el producto configurado para operar dispersion
                        tbpk = new TablaPK(idtabla, "producto_spei");
                        tabla = tablaService.buscarPorId(tbpk);
                        if (Integer.parseInt(tabla.getDato1()) == opa.getIdproducto()) {
                            clabe = clabeInterbancariaService.buscarPorId(a.getAuxiliarPK());
                        ...
                        clabe = clabeInterbancariaService.buscarPorId(a.getAuxiliarPK());
                        dispersion = validaReglasCsn(a, clabe.getClabe(), order, 1);
                        if(dispersion.getId() == 200) {
                            banderaProcesa = true;
                        }else{
                            log.info("::::::::::Sucedio un error en la validacion:::::::");
                        }
                    }
                    ...
                    if (banderaProcesa) {
                        if (clabe != null) {
                            ...
                            Banco banco = bancoService.buscarNombre(order.getBanco().trim());
                            if (banco != null) {
                                ...
                                log.info("::::::::::::::::Banco receptor no existe::::::::::::::" + order.getBanco() + ".......");
                                dispersion.setId(0);
                                dispersion.setError("Banco receptor no existe:" + order.getBanco());
                            }
                        } else {
                        ...
                        log.info("::::::Verifica datos como por ejemplo(Producto configurado,reglas en validacion):::::::");
                    }
    ...
    private ResponseLocalDispersionVo validaReglasFama(Auxiliar opa, String clabe, RequestLocalDispersionVo orden, Integer tipoProducto) {
        log.info(":::::::::::::::::::::::Validando regla para Fama ::::::::::::::::::::");
                                ...
                                if(opa.getSaldo().doubleValue() >= orden.getMonto()){
                                    if(opa.getEstatus() == 2){
                                       log.info("::::::::::Validacion exitosa:::::::::");
                                        ...
                                        log.info("::::::::::Estatus folio auxiliar invalido:::::::::.");
                                        validacion.setId(14);
                                        validacion.setError("Estatus invalido");
                                    }
                                }else{
                                   log.info(":::::::::::Saldo insuficiente para procesar la transaccion::::::::");
                                   validacion.setId(14);
...
private int procesarPoliza(Auxiliar opa,RequestLocalDispersionVo orden,String referencia, int tipoop) {
    int aplicados = 0;
    try {
...
private OrdenPagoWS validarFormarOrdenPago(RequestLocalDispersionVo orden) {
    ...
    //Se usa como clase de paso pero tambien se usa para generar la cadena a firmar
        ...
        if (auxiliar != null) {
            if (auxiliar.getEstatus() == 2) {
                ...
                if (persona != null) {
                    ...
                    Banco institucionDestino = bancoService.buscarNombre(orden.getBanco().trim());
...
private OrdenPagoWSIndirecto validarFormarOrdenPagoIndirecto(RequestLocalDispersionVo orden) {
    log.info("¡Vamos a validar los datos!");
    OpaDTO opa = util.opa(orden.getOpaCliente());

    //Se usa como clase de paso pero tambien se usa para generar la cadena a firmar
    OrdenPagoWSIndirecto ordenEnviada = new OrdenPagoWSIndirecto();
...
private STPDispersionResponseVo enviarOrdenHTTP(RequestLocalDispersionVo orden) {
    ...
    try {
        ...
        ordenSTP.setEmpresa(tabla.getDato1());// Solicitarlo a Caja
        ordenSTP.setInstitucionContraparte(orden.getBanco());
        ordenSTP.setInstitucionOperante("");// Solicitarlo a caja
        ordenSTP.setMonto(String.valueOf(orden.getMonto()));
        ordenSTP.setNombreBeneficiario(orden.getBeneficiario());
...
public String firmarEnviarOrdenPI(OrdenPagoWSIndirecto oPW) {
    StringBuilder sB = new StringBuilder();
    String firma = "";
    try {
        sB.append("||");
