...
--raise notice 'A) n_monto_mov: %,  n_suma_total: %', n_monto_mov, n_suma_total;
        if n_monto_mov > n_suma_total then

          n_sobrante := sai_spei_entrada_prestamo_adelanto_exacto (r_mov.idorigenp, r_mov.idproducto, r_mov.idauxiliar,
                                                                   p_fecha, n_monto_mov, n_suma_total);
--raise notice 'B) n_sobrante: %', n_sobrante;
          if (n_monto_mov - n_sobrante) <= 0 then
            delete
            from   spei_entrada_temporal_cola_guardado
            where  fecha_inserta = r_mov.fecha_inserta and idusuario = r_mov.idusuario and sesion = r_mov.sesion and
                   referencia = r_mov.referencia and idorigenp = r_mov.idorigenp and idproducto = r_mov.idproducto and
    ...
    if n_sobrante > 0 then  -- Sobrante ajustalo al mismo folio del cargo
      if i_prod_resum > 0 then
        ...
        if found then  -- En caso de que exista, realize el cargo inverso a si mismo
          insert
          into   spei_entrada_temporal_cola_guardado
        ...
        else           -- En caso de no existir, realiza el movimiento
          select
          into  r_paso *
          from  auxiliares
          where (idorigen,idgrupo,idsocio) = (r_mov.idorigen,r_mov.idgrupo,r_mov.idsocio) and
                idproducto = i_prod_resum and estatus in (0,2);
          if not found then
            i_idorigenp  := r_mov.idorigen;
            i_idauxiliar := sai_ahorro_crea_apertura (
                                            array[text(r_mov.idorigen),  text(r_mov.idgrupo), text(r_mov.idsocio),
                                                  text(r_mov.idorigen),  text(i_prod_resum),  text(p_fecha),
                                                  text(r_mov.idusuario), '0.00'] );
          else
            i_idorigenp  := r_paso.idorigenp;
            i_idauxiliar := r_paso.idauxiliar;
          end if;

          t_aux         := sai_auxiliar(i_idorigenp, i_prod_resum, i_idauxiliar, p_fecha);
          n_io_cal      := split_part(t_aux,'|',2)::numeric;
          n_saldodiacum := split_part(t_aux,'|',3)::numeric;

          insert
          into   spei_entrada_temporal_cola_guardado
                 (idusuario,      sesion,        fecha_inserta, referencia,
                  idorigen,       idgrupo,       idsocio,
                  idorigenp,      idproducto,    idauxiliar,    idoperacion,
                  esentrada,      acapital,      io_cal,        saldodiacum,   sai_aux)
          values (p_idusuario,    p_sesion,      date(p_fecha), p_referencia,
                  r_mov.idorigen, r_mov.idgrupo, r_mov.idsocio,
                  i_idorigenp,    i_prod_resum,  i_idauxiliar,  r_mov.idoperacion,
                  TRUE,           n_sobrante,    n_io_cal,      n_saldodiacum, t_aux);
        end if;
      else
        update spei_entrada_temporal_cola_guardado
...
---- ******************************************************************
      ...
      update spei_entrada_temporal_cola_guardado
      set    acapital  = x_acapital,
             io_pag    = x_io_pag,
             io_cal    = x_io_cal,
             im_pag    = x_im_pag,
             im_cal    = x_im_cal,
             ivaio_pag = x_ivaio_pag,
             ivaio_cal = x_ivaio_cal,
             ivaim_pag = x_ivaim_pag,
             ivaim_cal = x_ivaim_cal,
...
--------------------------------------------------------------------------------
-- FUNCION PARA FAMA, PARA RECICLAR CLABEs DE PRESTAMOS YA PAGADOS DE UN MISMO
-- SOCIO
--------------------------------------------------------------------------------

create or replace function busca_y_asigna_clabe_anterior (integer,integer,integer,integer,integer,integer)
returns varchar as $$
declare
  p_idorigen   alias for $1;
  p_idgrupo    alias for $2;
...
begin
  ...
  where (idorigenp,idproducto,idauxiliar) = (p_idorigenp,p_idproducto,p_idauxiliar) and
        (idorigen,idgrupo,idsocio) = (p_idorigen,p_idgrupo,p_idsocio) and estatus = 2; -- estatus < 3;
  if not found or x is NULL then x := 0; end if;

  if x <= 0 then
    return 'ERROR: EL FOLIO NO EXISTE, NO LE PERTENECE AL SOCIO O NO ESTA ACTIVO';
  end if;
  ...
  where (idorigenp,idproducto,idauxiliar) in
        (select idorigenp,idproducto,idauxiliar
         from auxiliares
         where idorigenp > 0 and
               idproducto in (select idproducto from productos where tipoproducto = 2) and
               idauxiliar > 0 and estatus in (3,4) and
               (idorigen, idgrupo, idsocio) = (p_idorigen, p_idgrupo, p_idsocio)
         union
         select idorigenp,idproducto,idauxiliar
         from auxiliares_h
         where idorigenp > 0 and
  ...
  order by clabe
  limit 1;
  if not found then clabex := NULL; end if;

  if clabex is NULL then return ''; end if;

  if length(clabex) != 18 then return ''; end if;

  x := 0;
  select into x count(*)
  from ws_siscoop_clabe
  where clabe = clabex and seleccionada = TRUE and asignada = TRUE and eliminada = FALSE;
  if not found or x is NULL then x := 0; end if;

  if x <= 0 then
    return 'ERROR: HAY DETALLES CON LAS CLABES DEL SOCIO, LLAME A SISTEMAS';
  end if;

  -- SE PASA EL FOLIO QUE USO ESA CLABE A HISTORIAL
  insert into ws_siscoop_clabe_interbancaria_h
    select idorigenp,idproducto,idauxiliar,clabe,fecha_hora
    from ws_siscoop_clabe_interbancaria
    where clabe = clabex;

  -- AHORA ESA CLABE SE ASIGNA AL NUEVO OPA DEL SOCIO
  update ws_siscoop_clabe_interbancaria
  set idorigenp=p_idorigenp, idproducto=p_idproducto, idauxiliar=p_idauxiliar, fecha_hora = NOW(),
      asignada = TRUE, activa = TRUE, bloqueada = FALSE
  where clabe = clabex;
